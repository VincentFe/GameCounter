...
export class Game {
  ...
  private static guid: string = Generator.generateGUID();
   ...
   */
  constructor(players: Player[] = [], name: string = "Unnamed Game", active: boolean = true) {
  ...
  addPlayer(player: Player | string): Result {
    const res: Result = this.findPlayerByName(typeof player === "string" ? player : player.name);
    if (res.parmSuccess()) {
  ...
  updatePlayerScore(name: string, score: number): Result {
    ...
    if (playerResult.parmSuccess()) {
      const player = playerResult.parmObject() as Player;
  ...
  findPlayerByName(name: string): Result {
    const result = new Result();
    const player: Player | undefined = this.players.find((p) => p.name === name);
    if (!player) {
      result.parmSuccess(false);
      result.parmErrorMessage("Player not found.");
      return result;
   ...
   */
  async saveToFile(baseDir: string): Promise<Result> {
    const result = new Result();
    try {
